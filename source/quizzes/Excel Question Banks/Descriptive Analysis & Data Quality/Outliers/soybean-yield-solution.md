### Question a) Label both axes and write a descriptive chart title.
x-axis label: Soybean yield (kg/ha)
y-axis label: Frequency
Title: Soybean Yield in West Lafayette from 1975 to 2024

### Question b) This histogram clearly shows outliers. What is one possible reason these outliers are present in the dataset?
- A sensor or machine was incorrectly calibrated, causing the low outliers
- An environmental factor caused lower yield in one area, for example, there was shade cast on a plot by a structure, lowering yield

### Question c) Your teammate recommends you delete all the outliers. Provide one argument why this is a good idea, and one argument why this is a bad idea.
- This is a good idea because this data was measured wrong, and should be excluded from analysis
- This is a bad idea because this data is still valueable to be included in analysis, as it represents some area with lower yield.

### Question d) Your team decides to create an underflow bin. Quickly sketch the new histogram, showing the underflow bin. Label the bin range of the underflow bin and the first bin to the right of it.
![Histogram sketch](soybean-yield-solution-img.jpeg)

### Question e) After a team discussion, you decide to delete 93 outliers. Which values will increase?
- [ ] sample size
- [x] mean
- [x] median
- [ ] mode
- [ ] standard deviation
- [ ] range

### Question f) Which values will decrease?
- [x] sample size
- [ ] mean
- [ ] median
- [ ] mode
- [x] standard deviation
- [x] range

### Question g) Which numerical value changed the most?
- [ ] sample size
- [ ] mean
- [ ] median
- [ ] standard deviation
- [x] range

Range changes the most, by approximetly 1300 or more. Sample size will change by exactly 93. Mean and median will increase slightly but certinly not by more than 1300. Standard deviation will also change, but on a much smaller magnitude as it is the average squared deviation from the mean.
